...
class TestDataLoader(unittest.TestCase):
    def test_load_and_process_issues(self):
        loader = DataLoader()
        issues_df = loader.load_and_process_issues()
        self.assertIsNotNone(issues_df)  # Check if data is loaded
        self.assertGreater(len(issues_df), 0)  # Ensure data is not empty
